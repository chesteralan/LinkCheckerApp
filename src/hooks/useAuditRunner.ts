...
export function useAuditRunner() {
  ...
  const startQuick = useCallback(async (urls: string[], checks: SelectorCheck[], config: { mode: string; batchSize: number; timeoutSecs: number }, originOverride?: string, urlPostfix?: string) => {
    setState({ running: true, run: null, progress: null })
  ...
  return { ...state, start, startQuick, cancel }
}
